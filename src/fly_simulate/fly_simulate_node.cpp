...
void fly_sim::up_down_fly(const ros::TimerEvent& event){
    ...
    else
    {
        ...
        if(fly_server_.isPreemptRequested() || !ros::ok()){
            ...
            mrs_uwb_sim::FlyTaskResult result;
            result.is_reached = true;
            fly_server_.setSucceeded(result);
        }

        mrs_uwb_sim::FlyTaskFeedback feedback;
...
void fly_sim::execute_cb(const actionlib::SimpleActionServer<mrs_uwb_sim::FlyTaskAction>::GoalConstPtr& goal){
